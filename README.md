...
Agent 生成初版并完成基础检查后，这一步就结束了。接下来请你先自己核查和修改网页，不要直接提交。
...
修改完成后，再回到菜单选择下一项功能，继续执行导入仓库、校验和 Pull Request 准备。
...
参与者的最终作品统一提交到 `html_output/<paper-slug>_<source>/`。提交时，请将用于展示的演示 PPT 统一命名为 `presentation.pptx`，放入同一作品目录并与完整网页项目一并提交。不要修改 `html_output/` 的目录名称，不要将项目直接放在仓库根目录，也不要把论文 PDF 或修改说明放入 `html_output/`。
